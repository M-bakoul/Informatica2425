...
il primo nodo(2+) quindi lo scorreva tutto.
    ...
    */
    ...
    // poi ho fatto un algoritmo molto piu facile e corto
    // senza errori e con un un solo while che trovava
    // l id controllando direttamente tutta la lista_canzoni
    // questo |
    //       \ /
    ...
    if (temp == NULL) {
        printf("Canzone non trovata\n"); // se l id non esiste nella lista
        return;
    }

    // creo un nodo temporaneo che e praticamente il nodo che
    // contiene l id che cercavamo
    Canzone* current = (Canzone*)malloc(sizeof(Canzone));
    *current = *temp;
    current->next = NULL;

    // ora la playlist
    // se la playlist e vuota aggiunge il nodo in lista senza 
    // dimenticarmi che e cirolare
    if (playlist->head == NULL) {
        playlist->head = current;
        current->next = playlist->head; //qui
        playlist->lunghezza++;
    } else {
        Canzone* c1 = playlist->head;
        // se la playlist non e vuota
        while (c1->next != playlist->head){
            c1 = c1->next; // scorro tutta la playlist con il nodo temporaneo
        }

        c1->next = current; // lo aggiungo nella playlist
        current->next = playlist->head; // e circolare la playlist quindi il nodo inserito visto che ultio punta all head
        playlist->lunghezza++;
    }
